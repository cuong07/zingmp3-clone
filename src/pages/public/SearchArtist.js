...
const SearchArtist = () => {
    ...
    return (
        <div className='px-[59px] mt-12 flex flex-col '>
            <h3 className='text-main-text text-lg font-semibold mb-4'>Nghệ Sĩ/OA</h3>
            <div>
                    ...
                    {artists?.map((item) => (
                        <div className='w-[20%] flex flex-col gap-2 '>
                            ...
                            <div className='flex flex-col gap-1 text-center'>
                                <span className='font-semibold text-sm text-main-text'>{item.name}</span>
                                <span className='text-[#ffffff80] text-sm'>{item.totalFollow > 1000 ? `${Math.round(item.totalFollow / 1000)}K quan tâm` : `${item.totalFollow} quan tâm`}</span>
                            </div>
