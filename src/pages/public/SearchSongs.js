    ...
    return (
        <div className="px-[49px] mt-[49px]">
            ...
            <div className="flex flex-col w-full">
                {songs?.map((item) => (
                    <div
                        className={`flex w-full px-2 py-2 hover:bg-[#333] items-center rounded-md cursor-pointer
                    ${curSongId === item.encodeId ? "bg-[#595959]" : ""}
                    `}
                        onClick={() => getSongIdHandler(item.encodeId)}
                        key={item.encodeId}
                    >
                        ...
                        <div className="w-1/3 text-center text-[#ffffff80] hover:decoration-slice">
                            <NavLink to={item?.album?.link?.split(".")[0]}>
                                {item?.album?.title}
                            </NavLink>
                        </div>
